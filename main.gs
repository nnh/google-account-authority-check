class GetGoogleAccountInformationAll{
  constructor(){
    ...
    this.passwordLengthCheckColName = 'パスワードの長さに関するコンプライアンス';
    this.passwordSafetyCheckColName = 'パスワードの安全度';
    this.targetHeaders = ['ユーザー', 'ユーザー アカウントのステータス', this.adminStatusColName, '2 段階認証プロセスの登録', '2 段階認証プロセスの適用', this.passwordLengthCheckColName , this.passwordSafetyCheckColName];
    this.outputSheetName = '全て';
    this.sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.outputSheetName); 
    this.outputFilename = 'ISF27-4 管理台帳レビュー記録 資料 Googleアカウント ' + Utilities.formatDate(new Date(), 'Asia/Tokyo', 'yyyyMMdd');
  }
  getInputData_(){
    ...
    // Edit the headers.
    const userLogEditHeader = rawUserLogs.map((x, idx) => idx == 0? x.map(y => y.replace(/ \[.*$/u, '')): x);
    const tempUserLogOnlyOutputCol = this.transposeFunction_(userLogEditHeader);
    const userLogOnlyOutputCol = this.getOnlyOutputCol_(tempUserLogOnlyOutputCol, this.targetHeaders);
  ...
  getOnlyOutputCol_(inputData, targetColNames){
    const onlyTargetCol = inputData.filter(x => targetColNames.indexOf(x[0]) > -1);
  ...
  */
  setValuesToSheet_(){
    ...
    this.sheet.clear();
    this.sheet.getRange(1, 1).setValue('出力日：' + Utilities.formatDate(new Date(), 'JST','yyyy/M/d'));
    this.sheet.getRange(2, 1, targetValues.length, targetValues[0].length).setValues(targetValues);
  ...
  outputPdf_(){
    ...
    const hiddenStatus = hideSheetNonTargetPrinting(SpreadsheetApp.getActiveSpreadsheet(), [this.outputSheetName]);
   ...
   */
  createOutputSheets_(){
    if (!SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.outputSheetName)){
      SpreadsheetApp.getActiveSpreadsheet().insertSheet();
      SpreadsheetApp.getActiveSpreadsheet().getActiveSheet().setName(this.outputSheetName);
      this.setTargetColumnsWidth_(SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.outputSheetName));
    }
  }
...
class GetGoogleAccountInformationPasswordPolicy extends GetGoogleAccountInformationAll{
  constructor(){
    ...
    this.sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.outputSheetName); 
    this.outputFilename = 'ISF27-5 パスワードポリシー遵守確認 GoogleWorkspace';
  }
  editInputData_(){
    const targetValues = this.getInputData_();
    const passwordLengthCheckCol = targetValues[0].indexOf(this.passwordLengthCheckColName);
    const passwordSafetyCheckCol = targetValues[0].indexOf(this.passwordSafetyCheckColName);
...
function getGoogleAccountInformationPasswordPolicy(){
  ...
  const GetGoogleInfoPasswordPolicy = new GetGoogleAccountInformationPasswordPolicy();
  // Password policy.
...
function onOpen(){
  SpreadsheetApp.getActiveSpreadsheet().addMenu('パスワードポリシー遵守確認', [{name:'PDF出力', functionName:'getGoogleAccountInformationPasswordPolicy'}]);
  SpreadsheetApp.getActiveSpreadsheet().addMenu('アクセス権管理棚卸', [{name:'シート出力', functionName:'getGoogleAccountInformationAdmin'}]);
}
